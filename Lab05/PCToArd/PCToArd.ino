...
void setup() {
  ...
  //initialize lcd
  lcd.begin(16, 2);
  // reserve200 bytes for the string
...
void loop() {
  ...
  if (stringComplete) {
    ...
    Serial.println(inputString);// empty the string
    inputString = "";// reset the flag
    stringComplete = false;
  }
}

/*SerialEvent is called when new data is received on the RX port This function is automatically called when the loop is called. If we add delays in loop we will also delay the showing of the result. */
void serialEvent() {
